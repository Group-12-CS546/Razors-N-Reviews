...
router.post("/login", async (req, res) => {
	...
	if (typeof req.body.username != "string") {
		...
		return res.render("users/login", {
			errorcode: errorcode,
			errors: errors,
			message: "Please enter username of type string",
		});
	}
	...
	if (!req.body.username.match(regex)) {
		...
		return res.render("users/login", {
			...
			errors: errors,
			message:
				"Only Alphanumeric username is allowed and username should be more than 4 characters ",
		});
	}
	...
	if (typeof req.body.password != "string") {
		...
		return res.render("users/login", {
			...
			hasErrors: hasErrors,
			errors: errors,
			message: "The entered password must be string only",
		});
	}
	var len;
...
router.post("/signup", async (req, res) => {
	...
	if (user == null) {
		...
		res.render("users/signup", {
			errorcode: true,
			message: "Username already exists with that username",
		});
	} else {
		if (user) {
			...
			res.render("users/signup", {
				message:
					"Either the username or password is invalid, Please signup again",
				title: "Signup",
				heading: "Signup",
			});
		}
