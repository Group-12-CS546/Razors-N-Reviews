...
const create = async function create(
	...
	if (subStringHttp == website.substr(0, subStringHttp.length)) {
		if (
			subStringCom == website.substr(website.length - 4, subStringCom.length)
		) {
			var strLength =
				website.length - (subStringHttp.length + subStringCom.length);
			if (strLength >= 5) {
...
const update = async function update(
	...
	const updateSalonInfo = await salonCollection.updateOne(
		{ _id: parsedId },
		{ $set: updatedSalonInfo }
	);
