...
module.exports = {
    async addComment(customerId, reviewId, commentText) {
        ...
        const updatedInfo2 = await customerCollection.updateOne({ _id: parsedId }, { $push: { commentIds: newComment._id } });
        //const newId = insertInfo.insertedId;
    ...
    async getCommentsForReview(reviewId) {
        ...
        //let parsedId = ObjectId(customerId);
        const revCollection = await reviews();
        const foundComment = await revCollection
    ...
    async removeComment(id) {
        ...
        try {
            ...
            const deletionInfoForCommentFromReview = await reviewCollection.updateOne({ _id: objreviewId }, { $pull: { comments: String(id) } });
        ...
        try {
            ...
            const deletionInfoForCommentFromCustomer =
                await customerCollection.updateOne({ _id: objcustomerId }, { $pull: { commentIds: id } });
    ...
    async updateComment(id, commentText) {
        ...
        const updatedInfo = await revCollection.updateOne({ _id: parsedId2 }, { $push: { comments: updatedCommentData.commentText } });
        if (updatedInfo.modifiedCount === 0) {
            throw "Could not update Review Collection with Review Data!";
        }
        ...
        const customerCollection = await customers();
        //const finComment2 = await this.getComment(newId2.toString());
        const updatedInfo2 = await customerCollection.updateOne({ _id: parsedId }, { $push: { commentIds: id } });
